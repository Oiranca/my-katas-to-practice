...
import { Book } from './potterKataWithClass';

describe('potter books buy', () => {
  test('buy one', () => {
    const books = new Book(['First Title']);
    const calculate = books.pay;
    expect(calculate).toBe(8);
  });

  test('buy two different books', () => {
    const books = new Book(['First Title', 'Second Title']);
    const calculate = books.pay;
    expect(calculate).toBe(15.2);
  });
  test('buy three different books', () => {
    const books = new Book(['First Title', 'Second Title', 'Third Title']);
    const calculate = books.pay;
    expect(calculate).toBe(21.6);
  });
